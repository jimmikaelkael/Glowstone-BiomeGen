...
public class NoiseMapLayer extends MapLayer {
    ...
    private static final int[] LANDS = new int[] {GlowBiome.getId(Biome.PLAINS), GlowBiome.getId(Biome.DESERT),
            GlowBiome.getId(Biome.EXTREME_HILLS), GlowBiome.getId(Biome.FOREST), GlowBiome.getId(Biome.ICE_PLAINS),
            GlowBiome.getId(Biome.ICE_PLAINS)};
    ...
    public int[] generateValues(int x, int z, int sizeX, int sizeZ) {
        ...
        for (int i = 0; i < sizeZ; i++) {
            for (int j = 0; j < sizeX; j++) {
                ...
                values[j + i * sizeX] = noise >= -0.2D ? LANDS[nextInt(LANDS.length)] : 0;
            }
        }
        return values;
    }
